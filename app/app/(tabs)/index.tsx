...
export default function DealsScreen() {
  ...
  const candidates = useMemo(() => filterDeals(products, region, query, { ...filters, lowOnly: false }, { targetCurrency: currency, rateSnapshot: prefs.rateSnapshot }), [currency, filters, prefs.rateSnapshot, products, query, region]);
  ...
  const filtered = useMemo(() => filterDeals(products, region, query, filters, { signals, targetCurrency: currency, rateSnapshot: prefs.rateSnapshot }), [currency, filters, prefs.rateSnapshot, products, query, region, signals]);

  useEffect(() => {
    if (!loading && products.length && region !== 'all' && !regionOptions.includes(region)) void setMarket({ region: 'all', currency });
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}><FlatList data={data} numColumns={2} keyExtractor={(item) => item.sku_id} columnWrapperStyle={styles.columns} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setScanAttempted(false); void reload(); }} tintColor={colors.pill} />} ListHeaderComponent={<View style={styles.header}>
    <View style={styles.top}><BrandLogo style={styles.logo} /><MarketPill region={region} currency={currency} label={`${b('market')}: ${prefs.regionLabel(region)}`} onPress={() => setMarketOpen(true)} /></View>
