...
function CurrentCard({ product, offers, saved, onToggleSave }: { product: CatalogProduct; offers: Product[]; saved: boolean; onToggleSave: () => void }) {
  const prefs = usePreferences(); const { currency } = useMarket(); const { colors, styles } = useBrowseStyles(); const b = (key: Parameters<typeof browseText>[1]) => browseText(prefs.language, key); const best = bestYearbookOffers(offers, currency === 'original' ? undefined : currency)[0];
...
function ArchiveCard({ product }: { product: YearbookArchiveStyle }) {
  const prefs = usePreferences(); const { currency } = useMarket(); const { styles } = useBrowseStyles(); const b = (key: Parameters<typeof browseText>[1]) => browseText(prefs.language, key); const best = bestYearbookOffers(product.offers, currency === 'original' ? undefined : currency)[0];
  if (!best) return null;
