...
import { bestYearbookOffers, brandLabel, comparableYearbookOffer, filterYearbookArchive, filterYearbookProducts, groupYearbookArchive, indexYearbookDeals, yearbookBrands, yearbookCategories, yearbookYear, yearbookYears, type YearbookArchiveStyle } from '../../lib/yearbook';
...
export default function YearbookScreen() {
  ...
  const items = useMemo<Item[]>(() => {
    if (scope === 'current') return sortCurrent(currentFiltered, sort, currency, prefs.rateSnapshot).map((value) => ({ kind: 'current', value }));
    return sortArchive(archiveFiltered, sort, currency, prefs.rateSnapshot).map((value) => ({ kind: 'archive', value }));
  }, [archiveFiltered, currency, currentFiltered, prefs.rateSnapshot, scope, sort]);
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}><FlatList data={items} numColumns={2} keyExtractor={(item) => item.kind === 'current' ? item.value.catalog_product_id : item.value.archive_id} columnWrapperStyle={styles.columns} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" ListHeaderComponent={<View style={styles.header}>
    <View style={styles.top}><Text style={styles.title}>{prefs.t('tabs.yearbook')}</Text><MarketPill region={region} currency={currency} label={`${b('market')}: ${prefs.regionLabel(region)}`} onPress={() => setMarketOpen(true)} /></View>
  ...
  </View>} renderItem={({ item }) => <View style={styles.item}>{item.kind === 'current' ? <CurrentCard product={item.value} offers={dealIndex.byCatalogId[item.value.catalog_product_id] || []} saved={watchlist.isModelSaved(item.value)} onToggleSave={(offers) => void watchlist.toggleModel(item.value, offers).then((accepted) => { if (!accepted) Alert.alert(prefs.t('deals.watchLimitTitle'), prefs.t('deals.watchLimitBody', { count: watchlist.freeLimit })); })} /> : <ArchiveCard product={item.value} />}</View>} ListEmptyComponent={<ScreenState title={prefs.t('yearbook.noMatchesTitle')} body={prefs.t('yearbook.noMatchesBody')} />} />
  ...
  <MarketSheet visible={marketOpen} title={b('market')} region={region} currency={currency} regions={regionOptions.map((value) => ({ value, label: prefs.regionLabel(value), currency: value === 'all' ? b('localCurrency') : REGION_CURRENCY[value] || '—' }))} currencies={([{ value: 'original', label: b('localCurrency') }, { value: 'CNY', label: 'CNY' }, { value: 'USD', label: 'USD' }, { value: 'EUR', label: 'EUR' }] as Array<{ value: CurrencyPreference; label: string }>)} ratesNote={b('ratesNote')} catalogNote={b('catalogMarketNote')} applyLabel={b('apply')} onApply={(nextRegion, nextCurrency) => void setMarket({ region: nextRegion, currency: nextCurrency })} onClose={() => setMarketOpen(false)} />
  </SafeAreaView>;
}

function CurrentCard({ product, offers, saved, onToggleSave }: { product: CatalogProduct; offers: Product[]; saved: boolean; onToggleSave: (offers: Product[]) => void }) {
  const prefs = usePreferences(); const { currency } = useMarket(); const { colors, styles } = useBrowseStyles(); const b = (key: Parameters<typeof browseText>[1]) => browseText(prefs.language, key); const selection = comparableYearbookOffer(offers, currency, prefs.rateSnapshot); const best = selection.offer;
  const open = () => best ? router.push({ pathname: '/product/[skuId]', params: { skuId: best.sku_id } }) : void Linking.openURL(product.source_url);
  return <Pressable style={styles.card} onPress={open}><View style={styles.photo}><DefaultImage category={product.categories.join(' ')} brand={product.brand} /><Status tone={best ? 'sale' : 'list'} text={best ? b('onSale') : b('listPrice')} /><Pressable accessibilityRole="button" accessibilityLabel={b('modelWatch')} style={{ position: 'absolute', top: 3, right: 3, width: 44, height: 44, alignItems: 'center', justifyContent: 'center', borderRadius: 22, backgroundColor: colors.onPhotoBadge }} onPress={(event) => { event.stopPropagation(); onToggleSave(offers); }}><Ionicons name={saved ? 'heart' : 'heart-outline'} size={17} color={saved ? colors.disc : '#202326'} /></Pressable></View><Text style={styles.brandMeta}>{product.brand} · {prefs.categoryLabel(product.categories[0] || 'other')}</Text><Text style={styles.name} numberOfLines={2}>{product.name}</Text><Text style={styles.price}>{best ? offerPriceLabel(offers, selection.comparableAcrossCurrencies, prefs) : formatRange(product, prefs)}</Text><Text style={styles.marketMeta}>{best ? `${best.region.toUpperCase()} · ${PLATFORM[best._platform]?.label || best._platform}` : `${product.country.toUpperCase()} · ${product.currency}`}</Text></Pressable>;
}

function ArchiveCard({ product }: { product: YearbookArchiveStyle }) {
  const prefs = usePreferences(); const { currency } = useMarket(); const { styles } = useBrowseStyles(); const b = (key: Parameters<typeof browseText>[1]) => browseText(prefs.language, key); const selection = comparableYearbookOffer(product.offers, currency, prefs.rateSnapshot); const best = selection.offer;
  if (!best) return null;
  return <Pressable style={styles.card} onPress={() => router.push({ pathname: '/product/[skuId]', params: { skuId: best.sku_id } })}><View style={styles.photo}><DefaultImage category={product.categories.join(' ')} brand={BRAND[product.brand_key].label} /><Status tone="archive" text={b('archiveFrom')} /></View><Text style={styles.brandMeta}>{BRAND[product.brand_key].label} · {prefs.categoryLabel(product.categories[0] || productCategory(best))}</Text><Text style={styles.name} numberOfLines={2}>{product.name}</Text><Text style={styles.price}>{offerPriceLabel(product.offers, selection.comparableAcrossCurrencies, prefs)}</Text><Text style={styles.marketMeta}>{best.region.toUpperCase()} · {PLATFORM[best._platform]?.label || best._platform}</Text></Pressable>;
}

function offerPriceLabel(offers: Product[], comparable: boolean, prefs: ReturnType<typeof usePreferences>) {
  const rows = bestYearbookOffers(offers);
  if (comparable) {
    const selected = comparableYearbookOffer(offers, prefs.currency, prefs.rateSnapshot).offer;
    return selected ? prefs.formatMoney(selected.sale_price, selected.currency, selected.symbol) : '';
  }
  return rows.slice(0, 2).map((offer) => `${offer.currency} ${prefs.formatOriginalMoney(offer.sale_price, offer.currency, offer.symbol)}`).join(' · ');
}

function Status({ tone, text }: { tone: 'sale' | 'list' | 'archive'; text: string }) { const { styles } = useBrowseStyles(); return <View style={[styles.status, tone === 'sale' ? styles.statusSale : tone === 'archive' ? styles.statusArchive : styles.statusList]}><Text style={[styles.statusText, tone === 'sale' ? styles.statusSaleText : tone === 'archive' ? styles.statusArchiveText : styles.statusListText]} numberOfLines={1}>{text}</Text></View>; }
function Segment({ active, label, onPress }: { active: boolean; label: string; onPress: () => void }) { const { styles } = useBrowseStyles(); return <Pressable style={[styles.segmentButton, active && styles.segmentActive]} onPress={onPress}><Text style={[styles.segmentText, active && styles.segmentTextActive]} numberOfLines={1}>{label}</Text></Pressable>; }
...
function sortArchive(rows: YearbookArchiveStyle[], sort: Sort, currency: CurrencyPreference, snapshot: RateSnapshot | null) { return [...rows].sort((a, b) => { const ao = comparableYearbookOffer(a.offers, currency, snapshot).offer; const bo = comparableYearbookOffer(b.offers, currency, snapshot).offer; const ap = ao ? sortablePrice(ao.sale_price, ao.currency, currency, snapshot) : null; const bp = bo ? sortablePrice(bo.sale_price, bo.currency, currency, snapshot) : null; return sort === 'price_asc' ? comparePrice(ap, bp, 1) || a.name.localeCompare(b.name) : sort === 'price_desc' ? comparePrice(ap, bp, -1) || a.name.localeCompare(b.name) : a.name.localeCompare(b.name); }); }
