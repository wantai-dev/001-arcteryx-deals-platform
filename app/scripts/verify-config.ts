...
assert.equal(easJson.cli?.appVersionSource, 'local', 'The release must use the committed local version source');
assert.equal(easJson.build?.production?.autoIncrement, false, 'production build must not increment the committed build number');
