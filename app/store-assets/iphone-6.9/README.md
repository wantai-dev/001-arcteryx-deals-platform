...
This is the candidate 1.2 screenshot plan and remains pending until the root task freezes the exact 1.2.0 / Build 14 source and signed artifact. Capture the visible UI only from that frozen candidate after device smoke and StoreKit price verification. Added overlay copy must match `store-metadata/next-version.json`. Do not use fixture data, fabricate prices, expose tester data, or imply that unlinked deals are absent from a current official catalog.
